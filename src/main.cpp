...
void setup() {
  ...
  // Network targets render FreeInkUI screens (the Wi-Fi list, keyboard and
  // progress views) on the render task. On S3 devices every network target
  // keeps the reader-sized render stack, without loading the rest of the
  // reader resources: Check for Updates and KOReader Authenticate were left on
  // the 8 KB network stack and overflowed it on the X4 Pro (#762). C3 devices
  // retain the smaller network stack to preserve their tighter internal-RAM
  // budget.
