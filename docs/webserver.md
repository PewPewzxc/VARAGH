...
- The mDNS fallback URL, usually `http://varagh.local/`
...
3. Open the URL shown on the reader. `http://varagh.local/` is preferred
...
2. Prefer `varagh.local` when available, but keep the displayed IP address as a fallback.
